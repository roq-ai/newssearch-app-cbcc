...
type ColumnType = ColumnDef<EducationalResourceInterface, unknown>;

interface EducationalResourceListPageProps {
...
export function EducationalResourceListPage(props: EducationalResourceListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getEducationalResources({
        ...
        order: params.order,
        searchTermKeys: ['title.contains', 'content.contains', 'source_link.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EducationalResourceInterface>>(
    () => `/educational-resources?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: EducationalResourceInterface) => {
    if (hasAccess('educational_resource', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/educational-resources/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/educational-resources/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="educational_resource" />
        </Flex>
