...
import { getEducationalResources } from 'apiSdk/educational-resources';
import { EducationalResourceInterface } from 'interfaces/educational-resource';
import { getFactChecks } from 'apiSdk/fact-checks';
import { FactCheckInterface } from 'interfaces/fact-check';

import { FactCheckListPage as ShowCaseList } from 'pages/fact-checks';
...
function DashboardPage() {
  ...
  const educationalResourceStat = useSWR<PaginatedInterface<EducationalResourceInterface>>(
    () => `/educational-resources`,
    () => getEducationalResources({ limit: 1, offset: 0 }),
  );

  const factCheckStat = useSWR<PaginatedInterface<FactCheckInterface>>(
    () => `/fact-checks`,
    () => getFactChecks({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: educationalResourceStat?.data?.totalCount,
      isLoading: educationalResourceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Fact Checks',
      result: factCheckStat?.data?.totalCount,
      isLoading: factCheckStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`fact-checks`} passHref legacyBehavior>
                  <Button
